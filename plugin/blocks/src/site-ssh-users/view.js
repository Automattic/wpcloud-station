( ( wpcloud ) => {
	wpcloud.hooks.addAction(
		...
		( button ) => {
			...
			const sshUserContainer = button.closest(
			...
			if ( ! section.classList.contains( 'is-open' ) ) {
				const openFormButton = sshUserContainer.querySelector(
					'[data-wpcloud-action="wpcloud_expanding_section_toggle"]'
				);
	...
	wpcloud.hooks.addAction(
		...
		( isOpen, section ) => {
			...
			const sshUserSection = section.closest(
				'.wpcloud-ssh-user-form--expanding-section'
			);
			if ( ! sshUserSection ) {
				return;
