...
function SiteDetailBlock( { attributes, setAttributes, className } ) {
	const { style, adminOnly, target, externalLink, url, label, download } = attributes;
			...
			<InspectorControls>
				<PanelBody label={ __( 'Settings' ) }>
					<LinkableDetailSelectControl
						...
						help={ __( 'Select a site detail to link to. Leave blank if using a custom URL.' ) }
					/>
					<TextControl
						label={__('Custom URL')}
						value={url}
						onChange={(newUrl) => setAttributes({ url: newUrl })}
						help={__('Add a custom URL for this link. Just use the path for internal links i.e. `/sites` ')}
					/>
					<SelectControl
						label={__('Link Style')}
						value={style}
					...
					<ToggleControl
						...
						onChange={(newVal) => setAttributes({ target: newVal ? '_blank' : '_self' })}
					/>
					<ToggleControl
						label={__('External Link')}
						checked={externalLink}
						onChange={(newValue) => setAttributes({ externalLink: newValue })}
						help={__('Marking a link as external will add an external link icon to the link')}
					/>
					<ToggleControl
						label={__('Download')}
						checked={download}
						onChange={(newValue) => setAttributes({ download: newValue })}
