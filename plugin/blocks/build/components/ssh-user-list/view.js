...
/*!*****************************************************!*\
  !*** ./blocks/src/components/ssh-user-list/view.js ***!
  \*****************************************************/
(wpcloud => {
  const sshUserList = document.querySelector('.wpcloud-block-site-ssh-user-list');
  function onSshUserAdded({
    user
  }) {
    const newForm = sshUserList.querySelector('form').cloneNode(true);
    newForm.querySelector('.wpcloud-block-site-detail__value').textContent = user;
  ...
  wpcloud.hooks.addAction('wpcloud_form_response_site_ssh_user_remove', 'site_ssh_user_add', onSshUserRemove);
  wpcloud.hooks.addAction('wpcloud_site_ssh_user_added', 'site_ssh_user_added', onSshUserAdded);
})(window.wpcloud);
/******/ })()
;
