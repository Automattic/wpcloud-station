...
class WPCloud_Metrics {
	 ...
	 * @param string $dimension The dimension. defaults to 'http_status'.
	 * @param array  $options The options.
	 *
	 * @return WP_Error|array The data
	 */
	public function fetch_data( string $metric, string $dimension = 'http_status', array $options = array() ): WP_Error|array {

		if ( ! $this->has_support_for( $metric ) ) {
			return new WP_Error( 'invalid_metric', 'Invalid metric' );
		}

		if ( ! $this->has_support_for( $dimension ) ) {
	...
	/**
	 * Check if a metric or dimension is supported.
	 *
	 * @param string $metric_or_dimension The metric or dimension.
	 *
	 * @return bool True if the metric or dimension is supported.
	 */
	public function has_support_for( string $metric_or_dimension ): bool {
		return array_key_exists( $metric_or_dimension, self::get_available_metrics() ) || array_key_exists( $metric_or_dimension, self::get_available_dimensions() );
	}

	/**
	 * Get available metrics.
	 *
	 * @return array The available metrics.
	 */
	 ...
	 *
	 * @return array The available dimensions.
	 */
