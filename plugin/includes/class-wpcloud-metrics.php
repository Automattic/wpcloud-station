...
class WPCloud_Metrics {
	 ...
	 *
	 * @param string|null $metric The metric.
	 *
	...
	public static function get_available_dimensions( $metric = null ): array {
		...
		} elseif ( ! empty( $metric ) && str_starts_with( $metric, 'mysql_' ) ) {
		...
		} elseif ( ! empty( $metric ) && str_starts_with( $metric, 'cgroup_' ) ) {
		...
		} elseif ( in_array( $metric, array( 'uniques', 'views' ), true ) ) {
