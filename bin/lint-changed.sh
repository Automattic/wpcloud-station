...
if [ -z "$JS_FILES" ]; then
  ...
  if [ -n "$EXISTING_JS_FILES" ]; then
    ...
    cd plugin && npx wp-scripts lint-js $(echo "$EXISTING_JS_FILES" | sed 's|plugin/||g') && cd ..
  else
...
if [ -z "$CSS_FILES" ]; then
  ...
  if [ -n "$EXISTING_CSS_FILES" ]; then
    ...
    cd plugin && npx wp-scripts lint-style $(echo "$EXISTING_CSS_FILES" | sed 's|plugin/||g') && cd ..
  else
